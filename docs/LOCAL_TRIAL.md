...
> **順番が重要です。** 「Webhook」のトグルは、手順 5 で Webhook URL を登録するまで
> グレーアウトしてオンにできません（「Webhookを利用するには、Messaging APIの設定画面で
> Webhook URL を設定してください」と表示されます）。
> 手順 5 を終えてからこの画面をリロードすると、押せるようになります。
...
| 症状 | 確認すること |
| --- | --- |
| 友だち追加しても何も返らない | 応答設定の **Webhook がオン**か（URL 登録前はオンにできません）。ngrok のターミナルに `POST /api/line/webhook 200` が出ているか |
